...
export default class LanguageDropdown extends Component {
  ...
  renderDropdown = () => {
    ...
    const dropdownRows = headerConfig.langDropdown.filter(el => el.shortcut !== selectedLang);

    return dropdownRows.map(el => <LanguageDropdownRow onSelect={this.onRowSelect} {...el}/>)
  }

  renderSelected = () => {
    const {selectedLang, isOpened} = this.state;

    const selected = headerConfig.langDropdown.filter(el => el.shortcut === selectedLang)

    return <div className={styles.selected} onClick={this.toggleDropdown}>
      <img className={styles.flag} src={selected[0].imgSrc} alt="flag"/>
      <span>{selected[0].shortcut}</span>
      <img className={[styles.arrow, isOpened ? styles.up : ''].join(' ')} src={arrow} alt="arrow"/>
    </div>
  }
  ...
  render() {
    ...
    return (
      <div className={styles.container}>
        {this.renderSelected()}
        {isOpened && <div className={styles.dropdown}>
          {this.renderDropdown()}
        </div> }
      </div>
    )
